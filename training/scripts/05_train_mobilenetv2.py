"""
05_train_mobilenetv2.py
========================
Train MobileNetV2 (transfer learning) on the fixed 30-class final dataset.

Controlled experiment with EfficientNetB0 (06_train_efficientnetb0.py):
  ...
  - Architecture-specific official preprocessing (mobilenet_v2.preprocess_input)
...
from tensorflow.keras.applications.mobilenet_v2 import (
    MobileNetV2, preprocess_input as mobilenet_preprocess,
)
...
def build_model(num_classes):
    """MobileNetV2 transfer-learning model (224x224, 30 classes)."""
    base = MobileNetV2(
        include_top=False,
        weights="imagenet",
...
def main():
    ...
    # 4) Class weights (inverse frequency, same as EfficientNetB0)
    class_weight = common.compute_class_weights("train", class_names)

    # 5) Datasets (architecture-specific preprocessing)
    train_ds = common.build_dataset("train", class_names, mobilenet_preprocess, augment=True)
    val_ds = common.build_dataset("val", class_names, mobilenet_preprocess, augment=False)
    test_ds = common.build_dataset("test", class_names, mobilenet_preprocess, augment=False)

    # 6) Build model
    print("\n[6/7] Building MobileNetV2 transfer-learning model...")
    ...
    base.trainable = True
    # Freeze early layers, fine-tune last 40 base layers (MobileNetV2)
